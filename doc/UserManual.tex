...
\section{Probe Extraction Tool: \textit{horses2probes}}

HORSES3D provides a tool, \textit{horses2probes}, to extract solution variables at user-defined point locations (probes) from a solution file (*.hsol). For each probe, the host element of the mesh (*.hmesh) is located and the solution is interpolated at the requested point using the element's nodal (Lagrange) basis. The result can be written either in Tecplot ASCII format (*.tec) or as a plain text file. Usage:

\begin{lstlisting}[language=bash]
  $ horses2probes control_file.probes
\end{lstlisting}

A control input file is required, with the keywords described in table \ref{tab:horses2probes}:

\begin{longtable}{|p{3.2cm}|p{9.5cm}|p{2.7cm}|}
\caption{Keywords for \textit{horses2probes}.} \label{tab:horses2probes} \\
\hline
\multicolumn{1}{|c|}{\textbf{Keyword}} & \multicolumn{1}{c|}{\textbf{Description}} & \multicolumn{1}{c|}{\textbf{Default value}} \\ \hline
\endfirsthead

\caption{Additional keywords for \textit{horses2probes}} \\
\hline
\multicolumn{1}{|c|}{\textbf{Keyword}} & \multicolumn{1}{c|}{\textbf{Description}} & \multicolumn{1}{c|}{\textbf{Default value}} \\ \hline
\endhead
