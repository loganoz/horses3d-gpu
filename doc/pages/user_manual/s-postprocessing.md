...
## Probe Extraction Tool: *horses2probes*

`HORSES3D` provides a tool, *horses2probes*, to extract solution variables at user-defined point locations (probes) from a solution file (*.hsol). For each probe, the host element of the mesh (*.hmesh) is located and the solution is interpolated at the requested point using the element's nodal (Lagrange) basis. The result can be written either in Tecplot ASCII format (*.tec) or as a plain text file. Usage:

```bash
  $ horses2probes control_file.probes
```

A control input file is required, with the keywords described in the table below:

| Keyword           | Description                                                                              | Default value |
|-------------------|-------------------------------------------------------------------------------------------|----------------|
| hmesh file        | Location of the mesh file (*.hmesh)                                                      |                |
| hsol file         | Location of the solution file (*.hsol)                                                   |                |
| probes file       | Location of the probes file, containing the coordinates of the points to be sampled      |                |
| output file       | Name of the output file                                                                  | probes.tec     |
| output format     | Output file format, either *tec* (Tecplot ASCII) or *txt* (plain text)                   | tec            |
| output variables  | Output variables separated by commas. Available: \(\rho\), \(u\), \(v\), \(w\), \(p\)     | rho,u,v,w,p    |

The probes file contains one probe per line, with the three coordinates of the point separated by spaces:
...
The output file contains, for each probe, its coordinates, the requested variables, and a *found* flag (1 if the probe lies inside the mesh, 0 otherwise; in the latter case the corresponding variable values are written as zero).
