...
read partitioning from files & \textit{LOGICAL}: Specifies if the solver should read the partitioning information from files instead of performing the mesh partitioning. When \texttt{.FALSE.}, the solver reads the mesh file in serial at preprocessing, partitions the mesh, and writes the partitioning information to files in \texttt{./MESH/partitioning\_NUMRANKS\_ranks} (where \texttt{NUMRANKS} is the number of ranks). When \texttt{.TRUE.}, the solver will directly try to read the files in that folder. If the files do not exist, the solver will throw an error. & .FALSE. \\ \hline
...
\subsection{Probes from a file}

Instead of (or in addition to) defining probes one-by-one with \texttt{\#define probe} blocks,
a set of probes can be defined in bulk through a plain text \texttt{.dat} file, each line
...
\begin{longtable}{|p{4cm}|p{10cm}|p{2.2cm}|}
	\caption{Keywords for probes from a file.} \label{tab:ProbesFileKey} \\
	\hline
	\multicolumn{1}{|c|}{\textbf{Keyword}} & \multicolumn{1}{c|}{\textbf{Description}} & \multicolumn{1}{c|}{\textbf{Default value}} \\ \hline
	\endfirsthead
	
	\caption{Keywords for probes from a file - continued.} \\
	\hline
	\multicolumn{1}{|c|}{\textbf{Keyword}} & \multicolumn{1}{c|}{\textbf{Description}} & \multicolumn{1}{c|}{\textbf{Default value}} \\ \hline
	\endhead
	
	file 	&
	\textit{CHARACTER}: Path to the \texttt{.dat} file defining the probes' coordinates. May optionally be wrapped in quotes (\texttt{"..."} or \texttt{'...'}); case is preserved. &
	\textbf{Mandatory Keyword} \\ \hline
	
	variables 	&
	\textit{CHARACTER}: Whitespace-separated list of variables to sample at every probe in the file. &
	\textbf{Mandatory Keyword} \\ \hline
\end{longtable}
...
Lines that are empty or start with \texttt{\#} are ignored; every remaining line holds the
\texttt{x y z} coordinates of one probe. For each of these probes a dedicated output file
\texttt{<solution\_file>.probe\_N.probe} is created (\texttt{N} being the probe's position in the
file), with one column per variable (as listed in \texttt{variables}) and one row per saved
time step (in addition to the \texttt{Iteration} and \texttt{Time} columns).

To keep the per-iteration screen log readable when many probes are defined this way,
file-based probes are excluded from it; their individual \texttt{.probe} files are still
...
\item UserDefinedTermination: Called at the end of the main driver after everything else is done.
\end{itemize}
...
\section{Probe Extraction Tool: \textit{probes2tecplot}}

HORSES3D provides a tool, \textit{probes2tecplot}, to extract solution variables at user-defined point locations (probes) from a solution file (*.hsol). For each probe, the host element of the mesh (*.hmesh) is located and the solution is interpolated at the requested point using the element's nodal (Lagrange) basis. The result can be written either in Tecplot ASCII format (*.tec) or as a plain text file. Usage:

\begin{lstlisting}[language=bash]
  $ probes2tecplot control_file.probes
\end{lstlisting}

A control input file is required, with the keywords described in table \ref{tab:probes2tecplot}:

\begin{longtable}{|p{3.2cm}|p{9.5cm}|p{2.7cm}|}
\caption{Keywords for \textit{probes2tecplot}.} \label{tab:probes2tecplot} \\
\hline
\multicolumn{1}{|c|}{\textbf{Keyword}} & \multicolumn{1}{c|}{\textbf{Description}} & \multicolumn{1}{c|}{\textbf{Default value}} \\ \hline
\endfirsthead

\caption{Additional keywords for \textit{probes2tecplot}} \\
\hline
\multicolumn{1}{|c|}{\textbf{Keyword}} & \multicolumn{1}{c|}{\textbf{Description}} & \multicolumn{1}{c|}{\textbf{Default value}} \\ \hline
\endhead

hmesh file 		& Location of the mesh file (*.hmesh) 	&  \\ \hline
hsol file 		& Location of the solution file (*.hsol) 	&  \\ \hline
probes file 		& Location of the probes file, containing the coordinates of the points to be sampled 	&  \\ \hline
output file 		& Name of the output file 	& probes.tec \\ \hline
output format 		& Output file format, either \textit{tec} (Tecplot ASCII) or \textit{txt} (plain text) 	& tec \\ \hline
output variables 	& Output variables separated by commas. Available: $\rho$, $u$, $v$, $w$, $p$ 	& rho,u,v,w,p \\ \hline
\end{longtable}
...
The output file contains, for each probe, its coordinates, the requested variables, and a \textit{found} flag (1 if the probe lies inside the mesh, 0 otherwise; in the latter case the corresponding variable values are written as zero).
