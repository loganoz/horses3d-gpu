...
#  Simplification is NOT required for correctness (fcode emits equivalent Fortran either
#  way); it only makes the expressions more compact and can be very slow. "none" is the
#  fast default; add "none" is handled in simp().
...
η_1, η_2   = sp.symbols('eta_1 eta_2')   # per-fluid dynamic viscosities
cs_1, cs_2 = sp.symbols('cs_1 cs_2')     # per-fluid speeds of sound

# Mixture density — matches the solver (rho = c*rho1 + (1-c)*rho2)
ρ_mix = c*ρ_1 + (1 - c)*ρ_2
sqrtρ = sp.sqrt(ρ_mix)

# Variable (c-dependent) viscosity, matching GetmTwoFluidsViscosity:
#   mu(c) = c*mu1 + (1-c)*mu2   (so the viscous divergence includes grad(mu) terms)
η = c*η_1 + (1 - c)*η_2

# Variable (c-dependent) speed of sound, matching the solver's
# use_non_constant_speed_of_sound branch:  invMa2 = (cs1*c + cs2*(1-c))^2 * rho
cs_eff = cs_1*c + cs_2*(1 - c)
...
rhs_exprs = [
    ...
    sp.diff(p,t) + cs_eff**2*ρ_mix*div(u,v,w),
]
...
!
            real(kind=RP) :: t
            real(kind=RP) :: rho_1, rho_2, eta_1, eta_2, cs_1, cs_2, M0, sigma, eps

#ifdef MULTIPHASE
            t     = time
            rho_1 = dimensionless_ % rho(1)
            rho_2 = dimensionless_ % rho(2)
            ! Per-fluid dynamic viscosities: mu(c) = c*eta_1 + (1-c)*eta_2
            eta_1 = dimensionless_ % mu(1)
            eta_2 = dimensionless_ % mu(2)
            ! Per-fluid speeds of sound (matches the solver's use_non_constant_speed_of_sound
            ! branch): cs_i = sqrt(invMa2(i)/rho(i)); invMa2 = (cs_1*c + cs_2*(1-c))^2 * rho
            cs_1  = sqrt(dimensionless_ % invMa2(1) / dimensionless_ % rho(1))
            cs_2  = sqrt(dimensionless_ % invMa2(2) / dimensionless_ % rho(2))
            M0    = multiphase_ % M0
